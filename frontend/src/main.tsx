...
function Landing({ setPage, connectWallet, wallets }: { setPage: (page: Page) => void; connectWallet: (role: Role) => void; wallets: Record<Role, string> }) {
  return <section className="hero page-rise"><div className="hero-copy"><p className="eyebrow">Research bounties with escrow discipline</p><h1>Fund questions. Reward useful answers.</h1><p>SourceBounty lets a bounty provider lock an Arc reward for a research question and lets bounty hunters submit answers for GenLayer-assisted review. Public citations are checked before they enter the verdict flow.</p><div className="actions"><button className="primary" onClick={() => setPage("provider")}>Create a funded bounty</button><button className="secondary" onClick={() => setPage("bounties")}>Find bounties</button></div><div className="auth-split"><button onClick={() => connectWallet("provider")}>Provider auth {wallets.provider && short(wallets.provider)}</button><button onClick={() => connectWallet("hunter")}>Hunter auth {wallets.hunter && short(wallets.hunter)}</button></div></div><div className="hero-panel"><span>Ask a precise question</span><span>Lock the reward</span><span>Submit cited answers</span><span>Review, release, or refund</span></div></section>;
}
